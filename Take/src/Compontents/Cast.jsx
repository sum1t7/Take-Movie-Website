...
const Cast = ({ cast }) => {
  ...
  return (
    <div className="flex flex-col gap-3 lg:px-22  px-10 bg-gray-900">
      {cast.cast.length != 0 && (<h2 class="text-2xl md:text-3xl font-bold flex items-center gap-3">
              ...
              {cast.cast.slice(0, 13).map((actor, index) => (
                ...
                >
                  <div className="relative">
                    <div className={`w-32 h-32 mb-3 rounded-full overflow-hidden transform transition-all duration-300 ${
                      hoveredIndex === index ? 'scale-105 shadow-lg shadow-fuchsia-500/20' : ''
                    }`}>
                      {loading ? (
